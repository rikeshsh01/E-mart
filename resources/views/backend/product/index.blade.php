                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            @endphp
                                            <th class="border-top-0"><img src="{{$photo[0]}}" alt="banner image" style="max-height:100px; max-width:120px;"></th>
                                            <th>${{number_format($items->price,2)}}</th>
                                            <th>{{$items->discount}}%</th>
                                            <th>{{$items->size}}</th>
                                            
                                            <th class="border-top-0">
                                                @if($items->conditions == 'new')
                                                <span class="badge badge-success">{{$items->conditions}}</span>
                                                @elseif($items->conditions == 'popular')
                                                <span class="badge badge-warning">{{$items->conditions}}</span>
                                                @else
                                                <span class="badge badge-primary">{{$items->conditions}}</span>
                                                @endif
                                            </th>
                                            ...
                                            <th>
                                                <a href="{{route('product.edit',$items->id)}}" data-toggle="tooltip" class="btn btn-sm btn btn-outline-warning" title="edit" data-placement="bottom"><i class="fas fa-edit"></i></a>
